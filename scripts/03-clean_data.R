...
#### Clean data ####
raw_data <- read.csv("~/Canadian-Bread-Pricing-Analysis/Bread-Pricing-Analysis/data/
                     01-raw_data/hammer-4-raw.csv")
product_data <- read_csv("~/Canadian-Bread-Pricing-Analysis/Bread-Pricing-Analysis/data
                         /01-raw_data/hammer-4-product.csv")
...
new_bread_data <- new_bread_data |> mutate(units = as.numeric(str_remove(units, "g"))) |>
  filter(!is.na(units))
new_bread_data <- new_bread_data |> filter(!is.infinite(current_price))
